...
public class MockBigtableImpl extends BigtableImplBase {
  ...
  public void generateInitialChangeStreamPartitions(
    ...
    if (response instanceof GenerateInitialChangeStreamPartitionsResponse) {
      ...
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method GenerateInitialChangeStreamPartitions,"
                      + " expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
  ...
  public void prepareQuery(
    ...
    Object response = responses.poll();
    if (response instanceof PrepareQueryResponse) {
      requests.add(request);
      responseObserver.onNext(((PrepareQueryResponse) response));
      responseObserver.onCompleted();
    } else if (response instanceof Exception) {
      responseObserver.onError(((Exception) response));
    } else {
      responseObserver.onError(
          new IllegalArgumentException(
              String.format(
                  "Unrecognized response type %s for method PrepareQuery, expected %s or %s",
                  response == null ? "null" : response.getClass().getName(),
                  PrepareQueryResponse.class.getName(),
                  Exception.class.getName())));
    }
  }
